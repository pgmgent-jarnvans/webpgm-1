...
(() => {
  const CSS_COLOR_NAMES = ['AliceBlue', 'AntiqueWhite', 'Aqua', 'Aquamarine', 'Azure', 'Beige', 'Bisque', 'Black', 'BlanchedAlmond',
  ...
  const app = {
    initialize () {
      ...
      window.addEventListener('keydown', (ev) => {
        this.changeBackgroundColorForElementBasedOnKey(this.boxElement, ev.key);
        console.log(ev.key);
      });
    },
    changeBackgroundColorForElementBasedOnKey (elem, key) {
      console.log(key);
      const colorsFromColorNames = CSS_COLOR_NAMES.filter(
        c => c.toLowerCase().charAt(0) == key.toLowerCase()
      );
      if (colorsFromColorNames.length >= 1) {
        const color = colorsFromColorNames[Math.floor(Math.random() * colorsFromColorNames.length)];
